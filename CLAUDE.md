...
8. [BUG] 重构版本中，算法部分能够正常处理输出结果，但是结果在会话时无法返回前端显示一直处于“思考中”，后端重启后、新建另一个对话后该对话才能显示结果。另外，新建会话后的推荐问题生成结果能够正常返回前端。查看浏览器控制台日志：onChatStop被反复触发。有可能是后端算法返回的是流式响应，在最后的时候发送了data:[DONE];前端未监测到[DONE]或未触发最终的渲染，导致UI一直停留在加载中。